...
def profilescreen():
    ...
    if selection=='0':
        while 1:
            ...
            if currentuser in proflist:
                print("Sorry, the username already exists. Please try another name or enter \'*\' to go back and choose an existing profile")
            elif currentuser=='*':
                profilescreen()            
            else:
...
def game():
    ...
    print('*'*75)
    time.sleep(3)
    print("\nThe number lies in between 1 and 100, take a guess ;-)\n")
    while guess!=answer:
